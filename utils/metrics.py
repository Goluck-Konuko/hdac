...
class Metrics:
	...
	def compute(self, org=None, dec=None, seq=None):
		self._presets(org, dec,seq)
		...
		if seq:
			return {'psnr':np.round(np.mean(self.s_psnr),3),
					'ssim': np.round(np.mean(self.s_ssim),3), 
					'ms_ssim': np.round(np.mean(self.s_ms_ssim),3)
			} #,
				#	'vif': np.round(np.mean(self.s_vif),3) , 
				#	'perp': np.round(np.mean(self.s_perp_loss),3)}
		else:
			return {'psnr':self.psnr, 
					'ssim': self.ssim, 
					'ms_ssim':self.ms_ssim,
					'vif':self.vif, 
					'perp':self.perp_loss} #, self.pim_loss
